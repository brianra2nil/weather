...
for (let i = 0; i < searches.length; i++) {
    ...
    itemElem.innerHTML = `
            ${searches[i].text}
            `
    document.getElementById('previous').append(itemElem) //this is the line that renders on screen

}
// new code from tutor, showing how to target classlist and call the api
document.addEventListener('click', (event) => {
   
    if (event.target.classList.contains('resultCity')) {
console.log('you clicked on a result city')
let clickedCity = event.target.innerHTML.trim()
...
// we created a function for the api and display of data so that we can just call it instead of typing it twice.  Function is named "api" and used on lines 28 and 51 
function api(city) {
